...
namespace UI_Test_TIMESERVICE
{
    internal class MainProcess
    {
        public void MainLogic()
        {
            ...
            // 2.1 Validate data employees, calendars
            if (!ValidateHelper.ValidateEmployees(employees) || !ValidateHelper.ValidateCalender(calendars))
            {
                LogHelper.Warn("DATA EMPLOYEE, HOLIDAY IS NULL, PROCESS RETURN");
                return;
            }
            ...
            if (list_date_difference == null) return;
            ...
            if (!DatabaseHelper.Inser_list_date(list_date_difference))
            {
                return;
            }
            transaction.Commit();
            ...
            if (list_date_insert_timesheet == null) return;
            ...
            if (lstLog.Count > 0)
            {
                ...
                transaction.Commit();
                LogHelper.Info("INSERT LOG DATA INTO DATABASE COMPLETE");
            }
            ...
            if (logs == null)
            {
                LogHelper.Error("GET LOG_TIME FROM DATABASE FAIL, PROCESS RETURN");
                return;
            }
        ...
        {
            // 8. Delete timsheet from table timsheet (day_of_year, timesheet_status = 0)
        ...
        private bool InsertTimeSheetToDB(List<Timesheet> lstTimeSheet, List<DateTime> dateTimes)
        {
            ...
            foreach (DateTime dt in dateTimes)
            {
                ...
                {
                    LogHelper.Info("Insert timesheetbyday " + dt.ToString("yyyy-MM-dd"));
        ...
        private List<Log> GetAllLog(List<DateTime> list_date_insert_timesheet, List<Employee> employees)
        {
            ...
            {
                LogHelper.Error("GET ALL LOG DATA ERROR!");
                LogHelper.Error(ex.Message);
                LogHelper.Error(ex.StackTrace);
            }
            return allLogs;
